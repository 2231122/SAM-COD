...
def validate(model, val_loader):
    ...
    cnt = 0
    with torch.no_grad():
        for image, mask, shape, name,_,_,mask1 in val_loader:
            image, mask,mask1 = image.to(device).float(), mask.to(device).float(),mask1.to(device).float()
...
def train(Dataset, Network, cfg, train_loss, start_from = 0):
    ...
    for epoch in range(start_from, cfg.epoch):
        ...
        batch_idx = -1
        image, mask,x_h,y_w,mask_gt = prefetcher.next()
        ...
        while image is not None:
            ...
            loss2, loss3, loss4, loss5, loss6 = train_loss(image, mask,x_h,y_w,mask_gt,net, dict(epoch=epoch+1, global_step=global_step, sw=sw, t_epo=cfg.epoch))
            ...
            image, mask,_,_,mask_gt = prefetcher.next()
            ta = time.time() - st
...
if __name__=='__main__':
    ...
    EXP_NAME = f'trained'
    cfg = dataset.Config(datapath=f'{root}', savepath=f'./out/{EXP_NAME}/', mode='train', batch=8, lr=1e-3, momen=0.9, decay=5e-4, epoch=total_epoch, label_dir = 'Scribble')
